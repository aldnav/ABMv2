...
public class Human extends Agent{
	...
	public static void main(String[] args) {
		...
		envi.pref.put("SER1-alpha", 7);
		envi.pref.put("SER2-alpha", 5);
		envi.pref.put("Infection Probability", 0.2);
		Human h = new Human(envi);
		
		Set<State> st = new HashSet<State>();
		st.add(State.SUSCEPTIBLE);
		st.add(State.SUSCEPTIBLE);
		st.add(State.INFECTED1);
		st.add(State.INFECTED2);
		h.setNeighbors(st);
		
		/**
		 * 1. Introduce selection for serotype path
		 * !2. Possible additional property for human is mark for selection n
		 * 3. Randomize path selection.
		 * */
		h.path = 2;
		int n = h.path;
		for (int i = 0; i < 60; i++) {
			h.updateState(n);
			System.out.println(h.state + " " + h.alpha + " " + h.beta);
		}
